...
module Pod
    class Installer
        ...
        def prebuild_watch_frameworks!
            ...
            # AK: Do not write back the Podfile.lock
            write_lockfiles

            local_manifest = self.sandbox.manifest
            UI::puts "#{local_manifest.pod_names}"
            ...
            targets = targets
                          .select {|pod_target| Pod::Platform::watchos.supports?(pod_target.platform) }
            ...
            if not Podfile::DSL.dont_remove_source_code
                ...
                to_remain_files = ['Manifest.lock', 'Podfile-watchOS', File.basename(existed_framework_folder)]
                to_delete_files = sandbox_path.children.select do |file|
                    filename = File.basename(file)
                    not to_remain_files.include?(filename)
                end
                to_delete_files += existed_framework_folder.children.select do |file|
                    file.to_s.end_with?('.dSYM') || file.to_s.end_with?('.pod_name')
                end
